...
func shake(node: CanvasItem, duration: float, magnitude: float, original_position: Vector2 = Vector2.ONE) -> Tween:
	var tween = node.create_tween()
	const shake_duration: float = 0.01
	
	var n_shakes: int = max(int(duration / (shake_duration * 2)), 1)
